...
namespace TurnAi.TournamentTests {
    ...
    public class AllOrderedPairsMatchMakerTests {
        ...
        [InlineData(6)]
        public void TotalNumberOfMatches(int numRobots) {
            int expectedNumMatches = numRobots * (numRobots - 1);
        ...
        [InlineData(5)]
        [InlineData(6)]
        public void NumberOfTotalMatchesForOnePlayer(int numRobots) {
            int expectedNumMatches = 2 * (numRobots - 1);
            int trackedRobotId = 2;
            var matchMaker = new AllOrderedPairsMatchMaker(numRobots);
            for (int i = 0; i < numRobots; i++) {
                matchMaker.AddWaitingRobot(i);
        ...
        [InlineData(5)]
        [InlineData(6)]
        public void NumberOfMatchesForOnePlayerAsFirst(int numRobots) {
            int expectedNumMatches = numRobots - 1;
            int trackedRobotId = 2;
            var matchMaker = new AllOrderedPairsMatchMaker(numRobots);
            for (int i = 0; i < numRobots; i++) {
            ...
            while (!matchMaker.IsFinished) {
                var match = matchMaker.GetNextMatch();
                if (match![0] == trackedRobotId) numMatches++;
                foreach (int robotId in match!) {
                    matchMaker.AddWaitingRobot(robotId);
                }
            }
        ...
        [Fact]
        public void NoAvailableMatch() {
            var matchMaker = new AllOrderedPairsMatchMaker(2);
            matchMaker.AddWaitingRobot(0);
            matchMaker.AddWaitingRobot(1);
            Assert.NotNull(matchMaker.GetNextMatch());
            Assert.NotNull(matchMaker.GetNextMatch());
